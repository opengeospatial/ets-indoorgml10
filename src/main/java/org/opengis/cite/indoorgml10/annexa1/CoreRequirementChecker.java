...
public class CoreRequirementChecker {
	 ...
	 * checkRequirement1.
	 * </p>
	 * @param fileURL a {@link java.net.URL} object
	 * @return a boolean
		...
		for (int q = 0; q < floors.size(); q++) {
			...
			for (int s = 0; s < ringsOnFloorPlan.size(); s++) {
				for (int t = 1; t < ringsOnFloorPlan.size(); t++) {
					...
					// System.out.println(s+" "+t+" result "+result); //Still under
					// development
	...
	public boolean checkRequirement4(URL fileURL) throws Exception {
		...
		for (int a = 0; a < spaceLayerList.getLength(); a++) {
			...
			NodeList nodesList = spaceLayer.getElementsByTagNameNS("http://www.opengis.net/indoorgml/1.0/core",
					"nodes");
			for (int b = 0; b < nodesList.getLength(); b++) {
				...
				for (int c = 0; c < stateMemberList.getLength(); c++) {
					...
					for (int d = 0; d < stateMemberChildNodesList.getLength(); d++) {
						if (stateMemberChildNodesList.item(d)
							.getClass()
							.toString()
							.endsWith("org.apache.xerces.dom.DeferredElementNSImpl")) {
							DeferredElementNSImpl state = (DeferredElementNSImpl) stateMemberChildNodesList.item(d);

							State stateInstance = new State(
									state.getAttributeNS("http://www.opengis.net/gml/3.2", "id"));
		...
		for (int f = 0; f < interLayerConnectionList.getLength(); f++) {
			...
			NodeList interConnectsList = interLayerConnection
				.getElementsByTagNameNS("http://www.opengis.net/indoorgml/1.0/core", "interConnects");
			...
			for (int sl = 0; sl < spaceLayers.size(); sl++) {
				if (spaceLayers.get(sl)
					.hasState(interConnects1.getAttributeNS("http://www.w3.org/1999/xlink", "href"))) {
			...
			for (int sl = 0; sl < spaceLayers.size(); sl++) {
				if (spaceLayers.get(sl)
					.hasState(interConnects2.getAttributeNS("http://www.w3.org/1999/xlink", "href"))) {
					spaceLayerContainingInterConnects2 = spaceLayers.get(sl).getGmlid();
				}
			}

			if (spaceLayerContainingInterConnects1.equals(spaceLayerContainingInterConnects2))
				result = false;
		}
